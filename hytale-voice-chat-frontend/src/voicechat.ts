...
export class VoiceChatController {
    ...
    private updatePeerState = (peerId: string, stateLabel: string) => {
        this.callbacks.onPeerListUpdate((prev) =>
            prev.map((peer) =>
                peer.id === peerId ? {...peer, state: stateLabel} : peer,
            ),
        )
    }
    ...
    private removePeerListItem = (peerId: string) => {
        this.callbacks.onPeerListUpdate((prev) =>
            prev.filter((peer) => peer.id !== peerId),
        )
    }
    ...
    private handleOffer = async (message: {
        from?: string
    ...
    private handleAnswer = async (message: {
        from?: string
        sdp?: RTCSessionDescriptionInit
    }) => {
        const peerId = message.from
    ...
    private handleIce = async (message: {
        from?: string
    ...
    private connectWebSocket = (token: string) => {
        ...
        const wsHost =
            window.location.port === '5173'
                ? `${window.location.hostname}:24454`
                : window.location.host
        const socketUrl = `${protocol}://${wsHost}/voice/ws`
        ...
        ws.addEventListener('message', async (event) => {
            ...
            switch (message.type) {
                ...
                case 'peer-leave':
                    ...
                    break
                case 'offer':
                    await this.handleOffer(message as {
                        from?: string
                        sdp?: RTCSessionDescriptionInit
                    })
                    break
                case 'answer':
                    await this.handleAnswer(message as {
                        from?: string
                        sdp?: RTCSessionDescriptionInit
                    })
                    break
                case 'ice':
                    await this.handleIce(message as {
                        from?: string
                        candidate?: RTCIceCandidateInit
                    })
                    break
                ...
                case 'error':
                    this.handleError(
                        new Error(
                            (message as { message?: string }).message || 'Voice chat error.',
                        ),
                    )
                    break
                default:
